...
A bookmarklet that opens a small panel on any web page, with chat and
helpers that only do what you tick on.

## Install

**→ [Go here to install](https://algchain.github.io/algkit/bookmarks/v1/)**

Drag the green **PlatoKit: Exit the Cave** button onto your bookmarks
bar. Then click it on any page.

## What it can do (only when you tick it on)

| | |
|---|---|
| Read page text | grabs the page (or your selection) and uses it as context for chat |
| Overlay an image | shows a static image on the page, doesn't capture clicks |
| Scroll the page | smooth-scrolls up or down |

Each one is a chip in the panel. Off by default. Untick it and the
helper can't do that anymore.

## Files in this folder

- [`bookmarklet.exit.src.js`](bookmarklet.exit.src.js) — the bookmarklet
  source. Read it once before installing; it never changes after that.
- [`index.html`](index.html) — the install page (what GH Pages serves).
- [`collar.html`](collar.html), [`collar.js`](collar.js) — the broker
  iframe that mediates between the bookmark and the helper apps.
- [`hub.html`](hub.html), [`hub.js`](hub.js) — sample helper UI.
- [`uapps/`](uapps/) — bundled helpers (chat, add-uapp, image demo).

## Audit

The whole point of a bookmarklet is that the body is frozen at install.
You read it once, then trust it forever. Updates land in `collar.js` at
the install origin, not in your bookmark.
